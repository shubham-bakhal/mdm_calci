        ...
        {data.map((item, index) => {
          return (
            ...
            >
              ...
              <td
                onClick={e => handleEdit(item.id, item.one, item.two)}
                className="px-6 py-4 text-lg"
              >
                {inEditMode.id === item.id ? (
                  <>
                    <input
                      type="number"
                      value={inEditMode.one}
                      onChange={e =>
                        handleChange(item.id, e.target.value, item.two)
                      }
                ...
                {inEditMode.id === item.id ? (
                  <>
                    <input
                      ...
                      onChange={e =>
                        handleChange(item.id, item.one, e.target.value)
                      }
                      className="w-20 text-black"
                    />
                  </>
